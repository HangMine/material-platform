 ...
 *
 */
export function query(target: Record<PropertyKey, unknown>, keys?: string | PublicKey[] | null, createTarget = false):
{ target: Record<PropertyKey, unknown>, key?: PublicKey | undefined, value: unknown } {
  if (keys == null || keys === '') {
  ...
  do {
    ...
    if (target && !Object.prototype.hasOwnProperty.call(target, key) && createTarget) {
      const createdTarget = typeof key === 'number' ? [] : {};
      // @ts-ignore
